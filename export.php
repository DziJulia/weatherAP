...
//if empty good to go with searching index also need to check if its in database
        if(empty($indexErr)){
            ...
            $display = False;
            ?>
            <form action="exported.php" method="POST" class="rounded" novalidate>
            <h1 class = "down text-center">Export weather data</h1>
             <!---INCLUDING FOOTER -->
                    ...
                    while($row = mysqli_fetch_assoc($select)) {
                        ...
                        <tr class="smaller">
                              ...
                              <?php  if(empty($row['name'])){
                                ...
                                <?php } ?>
                                <td><?php echo $row['id']; ?><input type="hidden" name="weatherRow" value="<?php echo $row['id']?>" ></td>
               ...
               ?>
                   ...
                   <br>
                        <div class="row d-flex align-items-end flex-column ">
                                <button id = "button" class="btn btn-lg" type="submit" name="delete" ><span class="far fa-trash-alt"></span> Delete</button> 
                            </div>
                            <!---END OF FORM -->
            </form>
               <?php
